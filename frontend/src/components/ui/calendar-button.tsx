...
const EVENT_DETAILS = {
    ...
    description: "A full day of cloud innovation, workshops, and networking with AWS Cloud Clubs. Connect with industry experts and student builders.",
    location: "AWS Headquarters, Conference Level 4",
    startTime: "2026-05-24T09:00:00Z",
    endTime: "2026-05-24T17:00:00Z",
};
...
const getGoogleCalendarUrl = () => {
    const startUTC = "20260524T090000Z";
    const endUTC = "20260524T170000Z";
...
const getOutlookCalendarUrl = () => {
    return `https://outlook.live.com/calendar/0/deeplink/compose?path=/calendar/action/compose&rru=addevent&subject=${encodeURIComponent(EVENT_DETAILS.title)}&startdt=2026-05-24T09:00:00Z&enddt=2026-05-24T17:00:00Z&body=${encodeURIComponent(EVENT_DETAILS.description)}&location=${encodeURIComponent(EVENT_DETAILS.location)}`;
};
...
const downloadIcs = () => {
    ...
    anchor.href = url;
    anchor.download = "aws-student-community-day.ics";
    anchor.click();
    window.URL.revokeObjectURL(url);
};

export const CalendarButton = ({ className }: { className?: string }) => {
    return (
        <Popover.Root>
            <Popover.Trigger asChild>
                <Button variant="ghost" size="lg" className={cn("h-14 w-full group overflow-hidden relative", className)}>
                    <motion.div
            ...
            <Popover.Portal>
                <Popover.Content
                    align="center"
                    sideOffset={8}
                    className="z-50 w-64 origin-top-right rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] p-2 shadow-elevated backdrop-blur-xl animate-in fade-in zoom-in duration-200"
                >
